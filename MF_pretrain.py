...
if __name__ == '__main__':
    ...
    if args.data_name == 'cifar10':
        ...
        train_dataset = dp.MySTL10('data', transform=Compose([ToTensor(), Normalize(0.5, 0.5)]), split='train',
                                   download=True, lamda=args.lamda)
        val_dataset = dp.MySTL10('data', split='test', download=True,
                                 transform=Compose([ToTensor(), Normalize(0.5, 0.5)]), lamda=args.lamda)
    ...
    for e in range(start_epoch, args.total_epoch):
        model.train()
        losses = []
        for img, mixup_label in tqdm(iter(dataloader)):
            step_count += 1
            img = img.to(device)
            mixup_label = mixup_label.to(device)

            predicted_img, mask = model(img)
            loss = torch.mean((predicted_img - mixup_label) ** 2 * mask) / args.mask_ratio
            loss.backward()
        ...
        with torch.no_grad():
            ...
            val_mix_tag = torch.stack([val_dataset[i][1] for i in range(48)])
            val_mix_tag = val_mix_tag.to(device)
            predicted_val_img, mask = model(val_img)
            predicted_val_img = predicted_val_img * mask + val_mix_tag * (1 - mask)
            # 创建一个图像网格
            grid = vutils.make_grid(val_img, nrow=8, padding=2, normalize=True)
            grid_1 = vutils.make_grid(val_mix_tag, nrow=8, padding=2, normalize=True)
            ...
            # img = torch.cat([val_mix_tag * (1 - mask), predicted_val_img, val_mix_tag], dim=0)
            ...
            vutils.save_image(grid_1.to(torch.device('cpu')), args.save_path + "2_image.png")
            vutils.save_image(grid_2.to(torch.device('cpu')), args.save_path + "3_image.png")
            vutils.save_image(grid_3.to(torch.device('cpu')), args.save_path + "4_image.png")
